    ...
    <div class="col-md-12">
        <div class="card shadow">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <div>
                    <a href="{{ route('perbandingan-alternatif.index.admin') }}" class="btn btn-secondary btn-sm">
                        ...
                        @for ($i = 0; $i < $count; $i++)
                            @for ($j = $i + 1; $j < $count; $j++)
                                ...
                                <div class="col-md-6 mb-3">
                                    <label class="form-label text-gray-900"><strong>{{ $alt1->nama_siswa }}</strong> vs <strong>{{ $alt2->nama_siswa }}</strong></label>
                                    <select name="nilai[{{ $alt1->id }}][{{ $alt2->id }}]" class="form-control">
                                        <option value="1" selected>1 - Sama penting</option>
                                        <option disabled>──────────────</option>
                                        <optgroup label="{{ $alt1->nama_siswa }} lebih penting">
                                            @for ($v = 2; $v <= 9; $v++)
                                                <option value="{{ $v }}">{{ $v }}</option>
                                            @endfor
                                        </optgroup>
                                        <optgroup label="{{ $alt2->nama_siswa }} lebih penting">
                                            @for ($v = 2; $v <= 9; $v++)
